...
$sql = selectWhere("*", 'topup', "date_format(top_date,'%Y-%m') = DATE_FORMAT(CURDATE(),'$date') order by top_date");
//show ในหน้า pdf
$tableh1 = "
<h3>วันที่พิมพ์รายงาน " . date("d/m/Y") . "</h3>
<h2 style='text-align:center'>Report Income</h2>
...
while ($row = $sql->fetch_assoc()) {
    $formattedAmount = number_format($row['top_amount'], 0, '.', ',');
    //show ในหน้า pdf
    $tablebody .= '
    <tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:3px;text-align:center;"  >' . $row['top_id'] . '</td>
        <td style="border-right:1px solid #000;padding:3px;text-align:center;">' . $row['top_cusid'] . '</td>
        <td style="border-right:1px solid #000;padding:3px;text-align:center;">' . $formattedAmount . '</td>
        <td style="border-right:1px solid #000;padding:3px;text-align:center;">' . $row['top_date'] . '</td>
...
/*
    ...
    <tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:3px;text-align:center;"  >' . $row['top_id'] . '</td>
        <td style="border-right:1px solid #000;padding:3px;text-align:center;">' . $row['top_cusid'] . '</td>
        <td style="border-right:1px solid #000;padding:3px;text-align:center;">' . $row['top_amount'] . '</td>
        <td style="border-right:1px solid #000;padding:3px;text-align:center;">' . $row['top_date'] . '</td>
