...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    where book_pubid = '$pubid' and book_status = '0' and (book_name like '%$Search%' or pub_name like '%$Search%')";
    ...
    where book_pubid = '$pubid' and book_status = '0' and (type_name like '%$Search%')";
    ...
    where book_pubid = '$pubid' and book_status = '0' and (tag_name like '%$Search%')";
    $ex_tag = connectdb()->query($sqltag);

    if($ex_book->num_rows > 0){
        while($row = $ex_book->fetch_array()){
            echo '<a href="display_search_draf.php?bookid='.$row['book_id'].'" class="list-group list-group-item-action border p-2">'.$row['book_name'].'</a>';
        }
    }
