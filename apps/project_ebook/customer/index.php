...
<body>
    ...
    $sql = "SELECT book_id , COUNT(receipt_detail.recd_bookid)
    FROM book
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $bookId = $row['book_id'];
            array_push($bookIds, $bookId); // เพิ่ม book_id เข้าไปในอาร์เรย์
        }
    } else {
        echo "0 results";
    }
    ...
    <div class="container px-4 px-lg-5 mt-3">
                    ...
                    for (let i = 0; i < response.recommendations.length; i++) {
                        ...
                        <div class="carousel-item ${active}">
                            <a href="search_content.php?bookid=${item.book_id}">
                                <img src="${item.book_cover}" class="d-block w-100" alt="${item.book_name}">
                            </a>
        ...
        <style>
            /* CSS to change the color of carousel control icons to black */
        ...
        <div id="carouselExampleControls" class="carousel slide" data-bs-ride="carousel" style="width: 45%; margin: 0 auto;">
        ...
        <h3>โปรโมชั่น</h3><a href="display_all.php">
            <h5>ดูทั้งหมด</h5>
        </a>
        <div class="row gx-4 gx-lg-5 row-cols-2 row-cols-md-3 row-cols-xl-4 ">
            <?php
            $sqlpro = "CALL GetPromotionBooks()";
        ...
        <h3>ขายดีสูงสุดประจำเดือน</h3><a href="display_all.php">
            <h5>ดูทั้งหมด</h5>
        </a>
        <div class="row gx-4 gx-lg-5 row-cols-2 row-cols-md-3 row-cols-xl-4 ">
            <?php
            $querybook = "CALL GetTopBooksByMonth()";
            $sqlbook = connectdb()->query($querybook);
            ...
            if ($sqlbook->num_rows > 0) {
                ...
                while ($row = $sqlbook->fetch_assoc()) {
                    ...
                    $sqlpro = "CALL GetSpecificPromotionBook('$bookid','$proid')";
                    $ex_pro = connectdb()->query($sqlpro);
                    if ($ex_pro->num_rows > 0) {
        ...
        <h3>มาใหม่</h3><a href="display_all.php">
            <h5>ดูทั้งหมด</h5>
        </a>
        <div class="row gx-4 gx-lg-5 row-cols-2 row-cols-md-3 row-cols-xl-4 ">
            <?php
            $query_new_book = "CALL GetNewBooks()";
            $sqlbook = connectdb()->query($query_new_book);
            if ($sqlbook->num_rows > 0) {
                while ($row = $sqlbook->fetch_assoc()) {
